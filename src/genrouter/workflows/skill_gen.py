...
class SkillGenWorkflow:
    name = "SkillGen"

    def __init__(self, skill_query: SkillQuery, rewrite: Rewrite, generate: Generate, analyze: Analyze) -> None:
    ...
    def run(self, prompt: str, generator: Any, config: dict[str, Any], prompt_id: str = "") -> WorkflowResult:
        ...
        skill_result = self.skill_query(prompt)
        rewrite_result = self.rewrite(prompt, evidence=[analyze_result.as_evidence()], skills=skill_result.instructions)
        generate_result = self.generate(rewrite_result.rewritten_prompt, generator)
        trace = [analyze_result.trace, skill_result.trace, rewrite_result.trace, generate_result.trace]
