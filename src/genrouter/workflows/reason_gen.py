...
from genrouter.workflows.search_plan import planned_reason_targets


class ReasonGenWorkflow:
    name = "ReasonGen"

    def __init__(self, reason: Reason, rewrite: Rewrite, generate: Generate, analyze: Analyze) -> None:
    ...
    def run(self, prompt: str, generator: Any, config: dict[str, Any], prompt_id: str = "") -> WorkflowResult:
        ...
        rewrite_result = self.rewrite(prompt, evidence=[analyze_result.as_evidence(), *reason_result.evidence])
        generate_result = self.generate(rewrite_result.rewritten_prompt, generator)
        trace = [analyze_result.trace, reason_result.trace, rewrite_result.trace, generate_result.trace]
